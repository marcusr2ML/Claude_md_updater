...
8. **Log the run only with permission.** Step 0 asks. On **yes**, fetch `LOGGING.md` and
   follow it for the rest of the install. On **no**, do not fetch it, do not create a log
   file, and do not mention logging again. Logging never blocks the install: if any part of
   it fails, carry on without it.
...
## Step 0 — Ask about root, and about logging

Two questions. Ask both before running anything.
...
Record the answer, then ask 0b.

### 0b — Logging

☐ **CONFIRM — may I record this install?**

> This guide is a work in progress, and several of its paths have never been run on hardware
> like yours. If you let me record what happens, the ALPS team can see which steps actually
> break and fix them.
>
> I would keep a transcript of the commands I run and their output — including the Step 1
> probe, which lists your compiler, Python and library versions. It is written to a plain
> text file on your Desktop. **Nothing is uploaded**; you can read it, edit it or delete it
> when we are done, and I will show you where it is.
>
> * **Yes** — record this install.
> * **No** — record nothing.

Honor the answer exactly.

**On yes**, fetch the logging instructions and follow them for the rest of the install:

```bash
mkdir -p "$HOME/Desktop"
export ALPS_LOG="$HOME/Desktop/alps-install-$(date -u +%Y%m%dT%H%M%SZ).log"
curl -sSfL -o "$HOME/Desktop/LOGGING.md" \
  https://raw.githubusercontent.com/<org>/<repo>/main/LOGGING.md
```

Read `LOGGING.md`, set up its wrapper, and use it from here on. If the fetch fails, tell the
user logging is unavailable and continue the install normally.

**On no**, do not run any of the above. Continue to the probe.

Then run the probe below either way: it is what picks the route *within* each branch.
